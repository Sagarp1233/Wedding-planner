...
import { COURT_MARRIAGE_SLUG, getStaticCourtMarriagePost, CourtMarriageGuideArticle } from './blog/courtMarriageGuide';
...
export default function BlogPostPage() {
  ...
  async function fetchPost() {
    try {
      ...
      async function tryDB(getter) {
        const { data: d, error: e } = await supabase.from('blogs').select('*').eq('slug', slug).eq('status', 'published').maybeSingle();
        return (!e && d) ? d : getter();
      }
      ...
      if (slug === COURT_MARRIAGE_SLUG) { const r = await tryDB(getStaticCourtMarriagePost); setPost(r); applyPostSEO(r); return; }
  ...
  if (post.slug === BUDGET_GUIDE_SLUG) {
    const readTime = 14;
    return (<IndianWeddingBudgetGuide2026Article post={post} readTime={readTime} copied={copied} onShare={handleShareURL} affiliateHref={affiliateHref} affiliateCtaLabel={affiliateCtaLabel} />);
  }
  if (post.slug === WEDDING_5L_SLUG) { return <WeddingUnder5LakhsArticle post={post} readTime={12} copied={copied} onShare={handleShareURL} affiliateHref={affiliateHref} affiliateCtaLabel={affiliateCtaLabel} />; }
  if (post.slug === LOW_BUDGET_PREMIUM_SLUG) { return <LowBudgetPremiumWeddingArticle post={post} readTime={13} copied={copied} onShare={handleShareURL} affiliateHref={affiliateHref} affiliateCtaLabel={affiliateCtaLabel} />; }
  if (post.slug === PHOTOGRAPHY_CHECKLIST_SLUG) { return <PhotographyChecklistArticle post={post} readTime={10} copied={copied} onShare={handleShareURL} affiliateHref={affiliateHref} affiliateCtaLabel={affiliateCtaLabel} />; }
  if (post.slug === BUDGET_CALCULATOR_SLUG) { return <BudgetCalculatorArticle post={post} readTime={11} copied={copied} onShare={handleShareURL} affiliateHref={affiliateHref} affiliateCtaLabel={affiliateCtaLabel} />; }
  if (post.slug === LAST_MINUTE_CHECKLIST_SLUG) { return <LastMinuteChecklistArticle post={post} readTime={12} copied={copied} onShare={handleShareURL} affiliateHref={affiliateHref} affiliateCtaLabel={affiliateCtaLabel} />; }
  if (post.slug === WHATSAPP_INVITE_SLUG) { return <WhatsAppInviteArticle post={post} readTime={11} copied={copied} onShare={handleShareURL} affiliateHref={affiliateHref} affiliateCtaLabel={affiliateCtaLabel} />; }
  if (post.slug === ARYA_SAMAJ_MARRIAGE_SLUG) { return <AryaSamajMarriageArticle post={post} readTime={16} copied={copied} onShare={handleShareURL} affiliateHref={affiliateHref} affiliateCtaLabel={affiliateCtaLabel} />; }
  if (post.slug === COURT_MARRIAGE_SLUG) { return <CourtMarriageGuideArticle post={post} readTime={17} copied={copied} onShare={handleShareURL} affiliateHref={affiliateHref} affiliateCtaLabel={affiliateCtaLabel} />; }
