...
import {
  WEDDING_5L_SLUG,
  getStaticWedding5LPost,
  WeddingUnder5LakhsArticle,
} from './blog/weddingUnder5Lakhs';
import {
  LOW_BUDGET_PREMIUM_SLUG,
  getStaticLowBudgetPremiumPost,
  LowBudgetPremiumWeddingArticle,
} from './blog/lowBudgetPremiumWedding';
import {
...
export default function BlogPostPage() {
  ...
  // Helper: try to load from DB first, fall back to static getter
  ...
  async function fetchPost() {
    try {
      ...
      // ── Static / enhanced articles ──────────────────────────────────────
      if (slug === BUDGET_GUIDE_SLUG) {
        const resolved = await fetchStaticPost(getStaticBudgetGuidePost);
        setPost(resolved);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="w-12 h-12 border-4 border-rose-gold/30 border-t-rose-gold rounded-full animate-spin" />
      </div>
    );
  }
  ...
  const affiliateCtaLabel =
    (post.affiliate_label && String(post.affiliate_label).trim()) || 'Learn more';

  // ── Enhanced article: Complete Indian Wedding Budget Guide 2026 ──────────
  ...
  // ── Enhanced article: Wedding Under ₹5 Lakhs ────────────────────────────
  ...
  // ── Enhanced article: Low Budget Premium Wedding Ideas ───────────────────
  ...
  // ── Enhanced article: Wedding Photography Checklist ──────────────────────
  ...
  // ── Generic ReactMarkdown renderer (all other DB posts) ──────────────────
  const wordCount = (post.content || '').split(/\s+/).filter(Boolean).length;
  const readTime = Math.ceil(wordCount / 200) || 1;

  const MarkdownComponents = {
    h1: ({ node, ...props }) => (
      <h1 className="text-3xl lg:text-4xl font-serif font-bold text-gray-900 mt-12 mb-6" {...props} />
    ),
    h2: ({ node, ...props }) => (
      <h2 className="text-2xl lg:text-3xl font-serif font-bold text-gray-900 mt-10 mb-5" {...props} />
    ),
    h3: ({ node, ...props }) => (
      <h3 className="text-xl font-serif font-bold text-gray-900 mt-8 mb-4" {...props} />
    ),
    p: ({ node, ...props }) => (
    ...
    a: ({ node, href, children, ...props }) => (
      <a
        {...props}
        href={href ? ensureHttps(href) : undefined}
    ...
    ul: ({ node, ...props }) => (
      <ul className="list-disc pl-6 text-lg text-gray-700 mb-6 space-y-2" {...props} />
    ),
    ol: ({ node, ...props }) => (
      <ol className="list-decimal pl-6 text-lg text-gray-700 mb-6 space-y-2" {...props} />
    ),
    li: ({ node, ...props }) => <li className="pl-2" {...props} />,
    blockquote: ({ node, ...props }) => (
      <blockquote
        className="border-l-4 border-rose-gold pl-4 italic text-gray-600 my-8 bg-rose-gold/5 py-4 pr-4 rounded-r-lg"
        {...props}
      />
    ),
    code: ({ node, inline, ...props }) =>
      inline ? (
        ...
        <pre className="bg-gray-900 text-gray-100 p-4 rounded-xl overflow-x-auto text-sm font-mono mb-6">
          <code {...props} />
        </pre>
      ),
      ...
      <main className="pt-24 pb-32 max-w-6xl mx-auto px-4 lg:px-8">
          ...
          {post.tags && (
            <div className="flex justify-center flex-wrap gap-2 mb-6">
              {post.tags.split(',').map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1 bg-rose-gold/10 text-rose-gold text-xs font-semibold rounded-full uppercase tracking-wider"
                >
                  {tag.trim()}
                </span>
              ))}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <article className="prose prose-lg max-w-none text-gray-800 animate-slide-up animation-delay-100 bg-white/95 backdrop-blur-sm rounded-3xl border border-gray-100 shadow-lg p-6 md:p-10 lg:p-12">
            ...
            {/* ── Inline CTA Banner ── */}
            <div className="my-10 p-6 sm:p-8 rounded-2xl bg-gradient-to-br from-rose-gold/5 via-white to-plum/5 border border-rose-gold/15 text-center not-prose">
          ...
          <aside className="mt-12 max-w-5xl mx-auto">
        ...
        <div className="mt-16 pt-8 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-6 max-w-5xl mx-auto">
          <div className="flex items-center gap-2 text-sm font-medium text-gray-600">
            ...
            >
              ...
              {copied && (
                <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[10px] px-2 py-1 rounded">
                  Copied!
                </span>
              )}
            </button>
          ...
          <Link
            to="/create-invitation"
            className="inline-flex items-center justify-center px-6 py-2.5 bg-gradient-to-r from-rose-gold to-plum text-white text-sm font-medium rounded-xl hover:shadow-lg hover:shadow-rose-gold/20 transition-all"
          >
            Create Wedding Invitation Free 💌
          </Link>
        </div>
      </main>
    </div>
